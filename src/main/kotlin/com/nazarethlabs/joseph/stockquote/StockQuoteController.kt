...
class StockQuoteController(private val stockQuoteService: StockQuoteService) {

    @Operation(
        summary = "Atualiza ou cria uma cotação pendente do dia",
        responses = [
            ApiResponse(responseCode = "201", description = "Cotação gerada com sucesso", content = [Content(mediaType = "application/json", schema = Schema(implementation = StockQuotePendingResponse::class))]),
