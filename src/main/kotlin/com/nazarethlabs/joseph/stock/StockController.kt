...
class StockController(private val stockService: StockService) {
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(responseCode = "201", description = "Ação criada com sucesso", content = [Content(mediaType = "application/json", schema = Schema(implementation = StockResponse::class))]),
            ApiResponse(responseCode = "400", description = "Dados de entrada inválidos", content = [Content()])
        ]
    )
    @PostMapping
    fun createStock(@RequestBody request: CreateStockRequest): ResponseEntity<StockResponse> {
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(responseCode = "200", description = "Ação encontrada", content = [Content(mediaType = "application/json", schema = Schema(implementation = StockResponse::class))]),
    ...
    @Operation(
        ...
        description = "Marca uma ação como excluída, preenchendo o campo `deletedAt`. O registro não é removido fisicamente do banco de dados.",
        responses = [
            ApiResponse(responseCode = "204", description = "Ação marcada como excluída com sucesso", content = [Content()]),
            ApiResponse(responseCode = "404", description = "Ação não encontrada", content = [Content()])
        ]
    )
    @DeleteMapping("/{id}")
